# Fraud Busters Command Center design system
...
workspace package (`@workspace/fraud-busters-command-center`): other artifacts depend
...
`@workspace/fraud-busters-command-center/components/...`. Never use a `@/` alias inside
...
## Source-derived usage guidance

The system is grounded in the Fraud Busters frontend architecture and system-design
documents supplied for the Q-Solve 2026 Kenya Hackathon. Apply these rules in every
consumer:

- Lead with the transaction-to-score workflow. Show the score, model identity,
  execution state, and recommended review action before technical detail.
- Use deep navy and sea-glass surfaces for calm operational scanning. Reserve amber
  for review states and coral for high-risk signals; always pair status color with
  text and an icon or glyph.
- Prefer plain language such as “review recommended,” “high-risk signal,” and
  “model score.” Never present a score as proof of criminal activity or an automatic
  enforcement decision.
- Keep QSVM visually identifiable while showing classical baselines with equal
  metric integrity. Missing or unavailable values must read as unavailable, not zero.
- Treat simulator, QPU, live, mock, cached, fallback, and unavailable as distinct
  states. Do not imply live hardware execution or quantum advantage without returned
  evidence.
- Optimize for projected readability: generous hierarchy, high-contrast labels,
  readable primary values, concise explanations, and accessible data-table
  alternatives for charts.
- Use synthetic or anonymized values only. Do not expose names, phone numbers,
  account identifiers, secrets, raw errors, or unnecessary transaction fields.
...
for `@workspace/fraud-busters-command-center` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/fraud-busters-command-center` as a `workspace:*` dependency,
...
- Building or styling web UI: `artifacts/fraud-busters-command-center/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/fraud-busters-command-center/docs/consuming-expo.md`
- Building or styling a slide deck: `artifacts/fraud-busters-command-center/docs/consuming-slides.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/fraud-busters-command-center/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/fraud-busters-command-center/docs/migrating-expo.md`
