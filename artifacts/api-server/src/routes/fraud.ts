...
router.post("/v1/analyze", (req, res): void => {
  ...
  const invalidFields = featureSchema
    .filter((field) => {
      ...
      if (field.type === "number") {
        return typeof value !== "number" || !Number.isFinite(value) || (field.min != null && value < field.min) || (field.max != null && value > field.max);
      }
  ...
  if (unsupportedKeys.length || missingKeys.length || invalidFields.length) {
    req.log.warn({ unsupportedKeys, missingKeys, invalidFields }, "Analysis feature validation failed");
    res.status(400).json({
